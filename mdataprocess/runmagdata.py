...
if net == 'regmex':
    ...
    for i in idx_daily:
        ...
        date_name_newf = convert_date(date_name,'%Y-%m-%d', '%Y%m%d')
        new_name = str(date_name_newf)[0:8]
        fname = f'{st}{date_name_newf}dmin.min'
        if not os.path.isfile(f'{path}{fname}'):
            fname = f'{st}{date_name_newf}pmin.min'
        #print(fname)
        fname2 = st+'_'+new_name+'m.dat'
...
if format =='XYZ':
    ...
    dat = {'X': X_noff1['2025-11-11':'2025-11-13 23:59:00'],
           'Y': Y_noff1['2025-11-11':'2025-11-13 23:59:00']}

    # Create DataFrames
    df = pd.DataFrame(dat).fillna(999.9)   # Ensure NaN replacement

    path = "/home/isaac/GICS/gic_rout/processed_data/"  
    full_path = f'{path}{st.upper()}_SG_20251111_20251113_dB.csv'
    df.to_csv(full_path, index=False, sep=',', header=False, float_format='%.3f')
    #masked = df.loc[ini:fin]
