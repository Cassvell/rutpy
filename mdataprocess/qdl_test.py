...
###############################################################################
#################################  File "/home/isaac/anaconda3/lib/python3.9/contextlib.py", line 126, in __exit__
 #   next(self.gen)##############################################
st= sys.argv[1]
...
fdate = idate + timedelta(days=26) + timedelta(hours=23) + timedelta(minutes=59)
...
for i in range(n):
    ...
    plt.plot(qdl_sm[i], color= color[i], label='QD'+str(i+1)+': '+qd)    
